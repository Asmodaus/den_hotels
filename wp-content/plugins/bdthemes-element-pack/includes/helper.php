...
if (!function_exists('bdt_license_validation')) {
    function bdt_license_validation() {
        ...
        \ElementPack\Base\Element_Pack_Base::addOnDelete(
            function () {
        ...
        $license_email = trim(get_option('element_pack_license_email', get_bloginfo('admin_email')));

        if (\ElementPack\Base\Element_Pack_Base::CheckWPPlugin($license_key, $license_email, $error, $responseObj, BDTEP__FILE__)) {
            return true;
        } else {
            return false;
        }
