...
function wpmobileapp_custom_loader() {
	...
	if (is_wpappninja() && theme_is_changed()) {
...
add_action( 'wp_enqueue_scripts', 'wpmobileapp_process_obstart', PHP_INT_MAX );
...
function wpmobileapp_modify_body($output) {
	...
	global $wpmobile_inject_head;
	$html = 'class="';
	if (wpappninja_iosstyle()) {$html .= 'ios';if (wpmobile_need_old_ios_support()) {$html .= ' with-statusbar';}}else{$html .= 'md';}
	$html .= '" ';
	if (substr(get_wpappninja_option('cache_type', 'networkonly'), 0, 4) == 'fast'){
