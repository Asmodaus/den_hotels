	...
	if ($certFile != '') {
		...
		for($i=0;$i<count($mini_ios);$i++) {
            ...
            if ( is_wp_error( $bypass ) ) {
                ...
                $response .= "(iOS): ok " . count($mini_ios[$i]);
            }
...
function wpmobileapp_send_push_mail( $args ) {
	...
	if (get_wpappninja_option('wpmobile_auto_mail') == '1') {
        ...
        if (!is_array($emailsto)) {
            $emailsto = explode(',', $args['to']);
        }

        foreach($emailsto as $emailto) {
            
            if (is_email($emailto) && $args['subject'] != "" && $args['message'] != "") {
                wpmobileapp_push($args['subject'], wp_strip_all_tags($args['message']), "", "", 'all', '', $emailto);
            }
        }
	}
