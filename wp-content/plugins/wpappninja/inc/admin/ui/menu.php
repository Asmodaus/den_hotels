...
if (isset($_POST)) {
    ...
    if (isset($_POST['wpappninja_app_name']) && isset($_POST['wpappninja_app_logo'])) {
        ...
        $_options['app']['splashscreen'] = sanitize_text_field($_POST['wpappninja_app_logo']);
    }
...
if (isset($_GET['supportoldios'])) {
    if ($_GET['supportoldios'] == '0') {
	    $__options = get_option( WPAPPNINJA_SLUG );
        $__options['supportoldios'] = '0';
	    update_option( WPAPPNINJA_SLUG, $__options );
    } elseif ($_GET['supportoldios'] == '1') {
	    $__options = get_option( WPAPPNINJA_SLUG );
	    $__options['supportoldios'] = '1';
	    update_option( WPAPPNINJA_SLUG, $__options );
    }
}
...
if (current_user_can( wpappninja_get_right() ) ) {
    ...
    if ((get_wpappninja_option('package', '') != '' && get_wpappninja_option('apipush', '') == '') || (get_wpappninja_option('appstore_package', '') != '' && get_wpappninja_option('appstore_package', '') != 'xxx' && get_wpappninja_option('appstore_package', '') != '' && get_option('wpappninja_pem_file', '') == '')) { ?>
...
<?php }
	...
	/*if (get_wpappninja_option('supportoldios', '') == '') { ?>
        <div class="wpappninja_help" style="border-left: 5px solid #c10033;background: #ffffd8;margin-bottom: 26px;"><?php _e('Check your iOS version, if the render is ok, ignore, if no, apply the fix and update your app from the wpmobile dashboard.', 'wpappninja'); echo ' <a style="display: inline-block;margin-left: 17px;font-size:17px;" href="?page=wpappninja_home&supportoldios=1">'; echo strtolower(__('Apply the fix', 'wpappninja'));?></a> <?php echo ' <a style="display: inline-block;margin-left: 17px;font-size:17px;" href="?page=wpappninja_home&supportoldios=0">'; echo strtolower(__('Ignore', 'wpappninja'));?></a></div>
	<?php }


	if (get_wpappninja_option('supportoldios', '') == '1') { ?>
        <div class="wpappninja_help" style="border-left: 5px solid #c10033;background: #ffffd8;margin-bottom: 26px;"><?php _e('When your app is updated on iOS, remove the fix.', 'wpappninja'); echo ' <a style="display: inline-block;margin-left: 17px;font-size:17px;" href="?page=wpappninja_home&supportoldios=0">'; echo strtolower(__('Remove the fix', 'wpappninja'));?></a> </div>
	<?php }*/


if (wpappninja_alert_old_basic()) { ?>
    <div class="wpappninja_help" style="border-left: 5px solid #c10033;background: #ffffd8;margin-bottom: 26px;"><?php _e('On December 15, 2016 notifications and statistics will no longer be available with the free package', 'wpappninja'); echo ' <a style="display: inline-block;margin-left: 17px;font-size:17px;" target="_blank" href="https://wpmobile.app/'; if (preg_match('#fr#', get_locale())) {echo 'prix';}else{echo 'en/price';}?>/?source=<?php echo get_bloginfo('url'); ?>/"><?php echo strtolower(__('UPDATE MY PLAN', 'wpappninja'));?></a></div>
<?php }
...
<div onclick="window.open('https://support.wpmobile.app/');" class="wpapp_menu_admin"><span class="dashicons dashicons-editor-help"></span> <?php _e('Help', 'wpappninja');?></div>
