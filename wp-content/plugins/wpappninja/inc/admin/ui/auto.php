...
function _wpappninja_display_auto_page() {
	...
	if (isset($demopage)) {
		...
		if ($postid != 0) {
			...
			$json['url'] = $url;
			$json['url_optimal'] = get_bloginfo('url') . '/' . "?iswpappninjaconfigurator=true&wpappninja_read_enhanced=" . $id;
			$json['id'] = $id;
...
<div class="wpappninja-builder">
	<div class="wpappninja-builder-left">
		<?php _e('Minimal role to view preview', 'wpappninja');?>
	</div>
	<div class="wpappninja-builder-right">

		<select name="<?php echo WPAPPNINJA_SLUG;?>[rightpreview]">

							<?php
							foreach ($right as $r => $n) {
								echo '<option value="' . $r . '" ';if (get_wpappninja_option('rightpreview', 'manage_options') == $r) {echo 'selected';}echo '>' . $n . '</option>';
							} ?>
...
<div class="wpappninja-builder" <?php if (get_wpappninja_option('appify') != '1' && get_wpappninja_option('wpappninja_main_theme', 'WPMobile.App') != 'WPMobile.App') {echo ' style="display:none"';} ?>>
	...
	<div class="wpappninja-builder-right">
		...
		<select name="<?php echo WPAPPNINJA_SLUG;?>[cache_type]">
			...
			<option value="networkonly" <?php if (get_wpappninja_option('cache_type', 'networkonly') == 'networkonly'){echo 'selected';} ?>><?php _e('Use cache system of the website (recommended)', 'wpappninja');?></option>
...
<br/><br/>
<label><input type="checkbox" value="on" name="<?php echo WPAPPNINJA_SLUG;?>[disable_all_cache]" <?php if (get_wpappninja_option('disable_all_cache', 'off') == 'on'){echo 'checked';} ?> /> <?php _e('[debug mode] Always refresh the app content', 'wpappninja');?></label>
...
<div class="wpappninja-builder" style="display:none">
	<div class="wpappninja-builder-left">
		<?php _e('Always refresh the app content (debug)', 'wpappninja');?>
	</div>
	<div class="wpappninja-builder-right">
...
<?php $orderby = get_wpappninja_option('orderby_list', 'post_date'); ?>
						...
						<div class="wpappninja-builder"><div class="wpappninja-builder-left"><?php _e('Item number', 'wpappninja');?></div><div class="wpappninja-builder-right">
								...
								<?php }	?>
<?php for($i = 1;$i < 10;$i++) { ?>
                                    <option value="<?php echo ($i*100);?>" <?php if (get_wpappninja_option('listnb', '10') == ($i * 100)){echo 'selected';} ?>><?php echo ($i * 100);?></option>
